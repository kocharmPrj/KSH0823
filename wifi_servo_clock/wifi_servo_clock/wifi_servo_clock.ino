...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      //cds1 = map(cds1, 0, 1023, 0, 100);
      //cds2 = map(cds2, 0, 1023, 0, 100);
      
      if ((cds2 >= 85) && cdsFlag && servoFlag)
      {
        cdsFlag = false;
        myservo.attach(SERVO_PIN);
        myservoTime = secCount;
        ...
        myservo.write(60);
        digitalWrite(LED_LAMP_PIN, LOW); //LED OFF

        sprintf(sendBuf, "[%s]CDS@%d@%d\n", LOGID, cds1, cds2);
        client.write(sendBuf, strlen(sendBuf));
        client.flush();
        strcpy(lcdLine2, "Outside Bright");
        lcdDisplay(0, 1, lcdLine2);
        //        digitalWrite(LED_BUILTIN_PIN, HIGH);     // LED 상태 변경
      } else if ((cds2 < 60) && !cdsFlag && servoFlag)
      {
        cdsFlag = true;
        myservo.attach(SERVO_PIN);
        myservoTime = secCount;
        ...
        myservo.write(0);
        digitalWrite(LED_LAMP_PIN, HIGH);   //실외가 너무 어두우면 블라인드 내리고 LED ON
        

        sprintf(sendBuf, "[%s]CDS@%d@%d\n", LOGID, cds1, cds2);
        client.write(sendBuf, strlen(sendBuf));
        client.flush();
        strcpy(lcdLine2, "Outside Dark");
        lcdDisplay(0, 1, lcdLine2);
...
void socketEvent()
{
  ...
  else if (!strcmp(pArray[1], "SENSOR") && servoFlag)
  {
    cds1 = atoi(pArray[2]);
    cds2 = atoi(pArray[3]);
